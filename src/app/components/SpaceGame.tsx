...
export default function SpaceGame() {
  ...
  const stateRef = useRef<"idle" | "playing" | "gameover" | "paused">("idle");
  ...
  const spawnWave = useCallback((waveNum: number, cw: number) => {
    ...
    const cols = isMobile
      ? Math.min(4 + waveNum, 6)
      : Math.min(6 + waveNum, 10);
  ...
  useEffect(() => {
    ...
    const drawPlayer = (x: number, y: number) => {
      if (invinceRef.current > 0 && Math.floor(Date.now() / 80) % 2 === 0)
        return;
      ctx.save();
    ...
    const loop = () => {
      ...
      if (stateRef.current !== "playing" && stateRef.current !== "paused") {
        ...
        if (stateRef.current === "gameover") {
          ...
          if (isMobile) {
            ctx.fillText(
              "Arraste para mover · atirador automático",
              w / 2,
              h / 2 + 18,
            );
      ...
      if (stateRef.current === "playing") {
        ...
        eBulletsRef.current = eBulletsRef.current.filter((b) => {
          b.y += b.speed;
          return b.y < h + 12;
        });
  ...
  }, [startGame, boom, spawnWave, togglePause]);
  ...
  return (
    <div ref={containerRef} className="w-full h-full relative select-none">
      ...
      <button
        onClick={togglePause}
        className="absolute top-6 right-2 z-10 p-2 text-primary hover:text-white bg-black/50 hover:bg-black/80 rounded-full transition-colors flex items-center justify-center"
        title="Pausar Jogo"
      >
      ...
      {isPaused && (
        <div className="absolute inset-0 z-20 bg-black/80 flex flex-col items-center justify-center backdrop-blur-sm">
          ...
          <p className="text-gray-300 mb-8 font-mono">
            Score Atual: {finalScore}
          </p>

          <div className="flex flex-col gap-4 min-w-[200px]">
            <button
              onClick={togglePause}
              className="px-6 py-3 bg-primary/20 border border-primary text-primary hover:bg-primary hover:text-black font-bold transition-all"
            >
              Continuar
            </button>
            <button
              onClick={resetGame}
              className="px-6 py-3 bg-secondary/20 border border-secondary text-secondary hover:bg-secondary hover:text-white font-bold transition-all"
            >
              Reiniciar
            </button>
            <button
              onClick={quitGame}
              className="px-6 py-3 bg-gray-800 border border-gray-600 text-gray-300 hover:bg-gray-700 hover:text-white font-bold transition-all"
            >
              Encerrar
            </button>
          </div>
